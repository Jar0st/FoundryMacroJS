...
async function changeMoney(targets,html)
{
  ...
  difference_money = divideValue(difference_money, 1); //targets.size);
  ...
  <table style "width: 100%; text-align:center; border: 1px solid black">
	...
	for(let target of targets)
	{
    let original_money = duplicate(target.data.data.currency);
    let update_money = changeValue(original_money,difference_money);    

    await target.update({"data.currency" : update_money});
  }
